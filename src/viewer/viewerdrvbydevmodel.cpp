...
ViewerDriversByDeviceModel::ViewerDriversByDeviceModel(QObject *parent)
...
void ViewerDriversByDeviceModel::buildTree() {
    ...
    for (int i = 0; i < allDevices.size(); ++i) {
        ...
        devicesByDriver[dev.driver].append(i);
    }
    ...
    for (auto it = devicesByDriver.constBegin(); it != devicesByDriver.constEnd(); ++it) {
        ...
        for (int idx : sortedIndices) {
            ...
            if (dev.subsystem == QStringLiteral("acpi")) {
                name = s::acpiDeviceNiceName(dev.devPath, rawName);
            } else {
...
QVariant ViewerDriversByDeviceModel::headerData(int section,
